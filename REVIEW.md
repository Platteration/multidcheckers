...
`BUG-1`, `REL-1`, `BUG-2`, `BUG-3`, `SEC-1`, `VER-1`

**Second pass** — the remaining medium findings and the low-severity ones that were trivial or small:

`VER-2`, `VER-3`, `SEC-2`, `SEC-3`, `BUG-4`

**Third pass** — a follow-up security audit found that the caps added for `SEC-3` bound the code and not the multiverse it replays into: a code inside both of them (65,508 characters, 605 actions) still built 502 timelines and 1,107 boards, and the autosave then replayed it at every launch. `decodeGame` now bounds the multiverse the replay builds (`MAX_TIMELINES`, `MAX_BOARDS`), not only the bytes and actions of the code, and `BUG-5` is closed from the other side: the map is windowed to the boards its viewport covers, so that same state draws 5,606 nodes rather than 111,679. The ceiling is on the import alone, deliberately. The autosave is the player's own game - every state of it reached one action at a time and drawn on the way - so discarding it at launch would delete their work, while refusing a pasted code costs them nothing they had; and with the code refused up front, an oversized multiverse can no longer reach the autosave at all. What bounds that path is `MAX_ACTIONS`, now applied to a stored list as well as a pasted one. (This paragraph also claimed that a game travelling at every chance "draws after 731 actions at 548 timelines and 1,279 boards, which is the most any legal action list can reach", replaying in under 250 ms. That is one line's draw and not a maximum; the fourth pass measured the real numbers and they are in `normaliseSaved`.) The validation half of `REL-2` is done too — the v1/v2 branch replays the actions out of a stored history instead of handing the states to React, and `cleanSetup` rebuilds the setup from known values on both branches — but not the escape hatch: there is still no ErrorBoundary in `App.tsx`, so a render crash from some other cause is still a crash at every launch. The pass also made `codeFromUrl` and `urlWithoutCode` a real inverse pair, so a code in a link shape the cleaner cannot rewrite is no longer read at all.

**Fourth pass** — an adversarial review of the third found that its import ceiling had been sized from bot games, which travel far less than a person does. At 96 timelines / 600 boards it refused a game played by hand after 122 actions, five rounds into taking every time travel on offer, and the sending end gave no sign: `GameScreen` built the code regardless and `ShareModal` left Copy and Share enabled, so both players ended up with a code neither could ever load. The ceiling is now measured from the play that grows fastest (450 timelines / 950 boards, crossed after 526 actions and 56.9 kB of code, just under what the 64 kB byte cap can deliver), and `shareOffer` applies every cap the loading end applies to the sender's own game, so a refusal arrives where the player can still see the game it is about. The same review found that `MAX_ACTIONS` applied to `looksLikeSavedGame` did not decline a long game but erased it — the record was refused at launch, a fresh game started, and the autosave wrote that fresh game's "nothing to save" over it. The cap is applied where the list is written now (`saveDecision`, which keeps the stored record and says so rather than clearing it), and a record that cannot be read is told apart from no record at all (`restoreSaved`), so the launch can no longer overwrite one. The map's windowing, the largest change of the third pass, had no test at all — both guards could be deleted with the suite green — and now has a render test that drives `onLayout` and `onScroll` on the real component; the band is also re-derived from the focused board when the scroll position stops meaning anything (a multiverse that shrank under it, or a layout that reported no width). Smaller: `codeFromUrl` and `urlWithoutCode` now read the same parse, so `<scheme>://load/<code>` — where the parser puts `load` in the host and the cleaner's rewrite can never reach it — is no longer read as a code, and the pair's inverse property is checked over 768 generated shapes rather than twelve listed ones; `cleanSetup` drops a `bot` in a mode that has none.
